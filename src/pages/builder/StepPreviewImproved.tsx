...
export default function StepPreviewImproved({ ctx }: { ctx: any }) {
  ...
  const replaceVariablesInText = (text: string, guestData: any): string => {
    ...
    result = result.replace(/\{\{first_name\}\}/g, guestData.name?.split(" ")[0] || "");
    result = result.replace(/\{\{last_name\}\}/g, guestData.name?.split(" ").slice(1).join(" ") || "");
                    ...
                    <div className="relative w-full h-full">
                      ...
                      <div className={`absolute inset-0 border border-gray-300 bg-white overflow-hidden ${
                        ...
                        <div className={`w-full h-full bg-white overflow-auto ${
                          activeDevice === 'mobile' ? 'h-[calc(100%-24px)]' : 
                          activeDevice === 'tablet' ? 'h-[calc(100%-24px)]' : 'h-full'
                        }`}>
                          {renderModelPreview()}
                        </div>
                      </div>
